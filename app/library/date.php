...
class Date
{
  ...
  public static function covertDateToSting($date = null) {
    ...
    $date = explode('-', $date);
    return (int)$date[2].' '.$this->dateRepo->getMonthName($date[1]).' '.($date[0]+543);
  }
  ...
  public static function covertDateTimeToSting($dateTime = null,$includeSec = false) {
    ...
    return (int)$date[2].' '.$this->dateRepo->getMonthName($date[1]).' '.($date[0]+543). ' '.(int)$time[0].':'.$time[1];
  }
  ...
  public static function calPassedDate($dateTime) {
    ...
    if($days == 0) {
      ...
      $passed = $this->dateRepo->covertDateTimeToSting($dateTime);
    }elseif($years == 1){
      $passed = $years.' ปีที่แล้ว';
    }elseif($months > 0){
      $passed = $months.' เดือนที่แล้ว';
    }elseif($days == 1){
      // $passed = 'เมื่อวานนี้ เวลา '.$this->dateRepo->covertTimeToSting($dateTime);
      $passed = 'เมื่อวานนี้';
    }else{
  ...
  //   if(!$this->dateRepo->isLeapYear($yearStart) && ((int)$date['month'] == 2) && ($date['day'] == 29)) {
  //     $start = $yearStart.'-'.$date['month'].'-28 00:00:00';
  //   }else{
  //     $start = $yearStart.'-'.$date['month'].'-'.$date['day'].' 00:00:00';
  //   }

  //   if(!$this->dateRepo->isLeapYear($yearEnd) && ((int)$date['month'] == 2) && ($date['day'] == 29)) {
  ...
  public function covertAnyFormatDateToSqlFormat($dateTime) {
