...
<div class="main-recommended-box min-vh-100">
  <canvas id="canvas_particle"></canvas>
  <div class="main-recommend-wrapper">
    <div class="main-recommend-jumbotron">
      <h1 class="ph1 ph5-ns">
        <div class="dn db-ns">
          รีสอร์ท บังกะโล โฮสเทล หรือธุรกิจอื่นๆที่ใกล้เคียง ทำให้ผู้เห็นและคนเข้าถึงได้อย่างง่าย ด้วยการเปิดร้านออนไลน์บนเว็บไซต์พร้อมการสนับสนุนจากเรา<br>หรือ หากคุณมี บัตรคอนเสิร์ต วอชเชอร์ และอื่นๆที่ไม่ได้ใช้แล้วสามารถนำมาขายได้ที่นี่
        </div>
        <div class="db dn-ns">
          แพ็กเกจ ที่พัก ท่องเที่ยว จากธุรกิจของคุณ หรือ บัตรคอนเสิร์ต ตั๋ว วอชเชอร์ และอื่นๆที่ไม่ได้ใช้แล้วสามารถนำมาขายได้ที่นี่
        </div>
      </h1>
      <hr class="w-60 w-30-ns center bg-white mv4">
      <div class="main-recommend-btn-group">
        @if(!$hasShop)
        <a href="/shop/create" class="btn btn-primary c-shadow-3">
          เปิดร้านเพื่อขยายธุรกิจพร้อมการสนับสนุนจากเรา
        </a>
        @endif
        <a href="/ticket/new" class="btn btn-secondary c-shadow-3">
          ลงขายหรือโฆษณาสินค้าของคุณตอนนี้
        </a>
      </div>
    </div>
  </div>
...
<div class="container-fliud">
  ...
  <div class="main-panel overflow-hidden">
    ...
    <div class="relative bg-color-green-radial-gradient overflow-hidden">
              ...
              <div class="ph5 pv4 pv5-ns relative zi-10">
                ...
                <div class="owner-section">
                  <div class="online-name">
                    <div class="online_status_indicator_{{$recommendedShop['id']}} online-status-indicator @if($recommendedShop['owner']['online']) is-online @endif"></div>
                    @if($recommendedShop['owner']['online'])
                      <small class="white ml4">ออนไลน์</small>
                    @else
                      <small class="white ml4">ออนไลน์ล่าสุด {{$recommendedShop['owner']['last_active']}}</small>
                    @endif
                  </div>
                </div> -->
    ...
    <hr class="w-90 center bg-light-silver mt0 mb0 db dn-ns">
    <hr class="w-90 center bg-white mt0 mb0 dn db-ns">
...
<script type="text/javascript">
  ...
  class ParticleEffect {
    ...
    resizeCanvas() {
      ...
      //   this.canvas.width = window.innerWidth - 290;
      // }else {
    ...
    runCanvas() {
      ...
      for (var i = j = 0, ref = this.numberOfParticles - 1; j <= ref; i = j += 1) {
        x = Math.random() * this.ctx.width;
        y = Math.random() * this.ctx.height * 2;
        var particle = new Particle(this.ctx, x, y);
        particle.energy = Math.random() * (particle.band / 256);
        particles.push(particle);
      }

      var j, len, particle, ii = 0;
      ...
      var handle = setInterval(() => {
        ...
        for (j = 0, len = particles.length; j < len; j++) {
          ...
          if (particle.y < -particle.size * particle.level * particle.scale * 2) {
            ...
            if(++ii == this.numberOfParticles) {
              clearInterval(handle);
              this.ctx.clearRect(0, 0, this.ctx.width, this.ctx.height);
              this.runCanvas(this.ctx);
              break;
            }

            particle.reset();
            particle.x = Math.random() * this.ctx.width;
            particle.y = this.ctx.height + particle.size * particle.scale * particle.level;
          }
          particle.move();
  ...
  $(document).ready(function(){
    ...
    const particleEffect = new ParticleEffect(document.getElementById('canvas_particle'));
    particleEffect.init();
